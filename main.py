...
def compute_momentum(df, n=3):
    df["momentum"] = df["close"] - df["close"].shift(n)
    return df

def compute_volatility(df, n=14):
...
async def train_model():
    ...
    features = ["ema10","ema50","rsi","macd","bb_upper","bb_lower","obv","momentum","volatility"]
...
def ml_predict(latest_row):
    if not ml_trained:
        return "neutral", 50
    features = ["ema10","ema50","rsi","macd","bb_upper","bb_lower","obv","momentum","volatility"]
    X = scaler.transform(np.array([latest_row[features]]))
...
async def send_signal(user_id, asset):
    ...
    if dir_rule==dir_ml and dir_rule!="neutral":
        direction=dir_rule
        accuracy=int((acc_rule*0.3 + acc_ml*0.5 + abs(news_score)*0.2*100))
    else:
        direction=dir_rule
        accuracy=int(acc_rule*0.3 + acc_ml*0.5 + abs(news_score)*0.2*100)
